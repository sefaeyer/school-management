...
public class MeetService {
    ...
    public ResponseMessage<MeetResponse> saveMeet(HttpServletRequest httpServletRequest, MeetRequest meetRequest) {
        //String username = (String) httpServletRequest.getAttribute("username"); // direk asagida methodun icine koyduk.
                                                                                  // variable uretmeye gerek kalmadi.
        User advisoryTeacher = methodHelper.isUserExistByUsername((String) httpServletRequest.getAttribute("username"));

        // request gonderen Teacher Advisor mi kontrolu:
        methodHelper.checkAdvisor(advisoryTeacher);

        // yeni kaydedilen meet saatlerinde cakisma var mi?
        dateTimeValidator.checkTimeWithException(meetRequest.getStartTime(),meetRequest.getStopTime());

        // Teacher in eski meet leri ile cakisma var mi?
        ...
        for(Long studentId : meetRequest.getStudentIds()){
            ...
            // ogrencinin daha onceki meet leri ile cakisma var mi?
        ...
        // EXTRA ! -> ogrencilerin mail adreslerine mail gonderiliyor
        // sendMailToService(savedMeet, "created a new meeting", students)

        return ResponseMessage.<MeetResponse>builder()
                .message(SuccessMessages.MEET_SAVE)
                .object(meetMapper.mapMeetToMeetResponse(meet))
                .httpStatus(HttpStatus.OK)
                .build();
    }
